...
class MVTecDatasetWithSAM(Dataset):
    def __init__(
        ...
        # 创建保存图像的目录
        self.save_dir = os.path.join("saved_images", classname)
        os.makedirs(self.save_dir, exist_ok=True)
        self.image_saved = False  # 标记是否已保存图像
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            if self.classname in SPECIAL_CATEGORIES:
                ...
                # 保存原始图像、SAM掩码和合成异常图像（只保存一次）
                if not self.image_saved:
                    # 保存原始图像
                    image.save(os.path.join(self.save_dir, "original.png"))
                    
                    # 保存SAM掩码
                    sam_mask.save(os.path.join(self.save_dir, "sam_mask.png"))
                    
                    # 保存合成异常图像
                    aug_image.save(os.path.join(self.save_dir, "augmented.png"))
                    
                    # 保存噪声掩码
                    aug_mask_img = Image.fromarray((aug_mask[0] * 255).astype(np.uint8))
                    aug_mask_img.save(os.path.join(self.save_dir, "noise_mask.png"))
                    
                    print(f"Saved sample images for {self.classname} to {self.save_dir}")
