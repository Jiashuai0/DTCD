...
from model.destseg_old import DeSTSeg, EnhancedDeSTSeg, FineTunedClassifier
...
def evaluate(args, category, model, visualizer, global_step=0):
    ...
    with torch.no_grad():
        # 获取类别预测
        cls_pred = torch.argmax(classifier(img), dim=1)
        # 根据预测结果动态加载参数
        model._load_category_params(ALL_CATEGORY[cls_pred.item()])
...
def load_component(model, checkpoint_path, component_name):
    """加载指定组件权重"""
    checkpoint = torch.load(checkpoint_path)
    # 处理组件前缀（如decoder.deconv1等）
    component_dict = {k.replace(f"{component_name}.", ""): v 
                     for k, v in checkpoint.items() 
                     if k.startswith(component_name)}
    getattr(model, component_name).load_state_dict(component_dict)
    print(f"Successfully loaded {component_name} from {checkpoint_path}")
...
def test(args, category):
    ...
    model = DeSTSeg(
        dest=True, 
        ed=True,
        shared_encoder=True,  # 启用共享编码器模式
        category_specific=True  # 启用类别特定解码器
    ).cuda()

    # 加载共享组件（编码器）
    shared_checkpoint = os.path.join(args.checkpoint_path, "shared_encoder.pth")
    if not os.path.exists(shared_checkpoint):
        raise FileNotFoundError(f"Shared encoder checkpoint not found at {shared_checkpoint}")
    load_component(model, shared_checkpoint, "encoder")

    # 加载类别特定组件
    specific_components = ['decoder', 'segmentation_net']
    for component in specific_components:
        component_path = os.path.join(args.checkpoint_path, f"{category}_{component}.pth")
        if not os.path.exists(component_path):
            raise FileNotFoundError(f"{component} checkpoint for {category} not found at {component_path}")
        load_component(model, component_path, component)

    # 验证模型完整性
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Loaded model for {category} | Total trainable params: {total_params/1e6:.2f}M")
