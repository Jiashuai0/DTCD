...
def apply_sam_mask(output_segmentation, sam_mask):
    """应用SAM前景掩码，背景区域置0"""
    # 确保掩码是二值的 (0或1)
    sam_mask_binary = (sam_mask <=0.45).float()
    # 应用掩码：前景保持不变，背景置0
    return output_segmentation * sam_mask_binary

def create_fusion_image(original_img, anomaly_map, sam_mask):
    """创建融合图像：将异常区域覆盖到原始图像上"""
    # 反归一化原始图像
    mean = np.array(NORMALIZE_MEAN).reshape(1, 1, 3)
    std = np.array(NORMALIZE_STD).reshape(1, 1, 3)
    original_img_denorm = original_img.permute(1, 2, 0).cpu().numpy()
    ...
    # 将SAM掩码转为二值掩码
    sam_mask_np = sam_mask.squeeze().cpu().numpy() > 0.5
    
    # 创建融合图像 - 只在SAM前景区域显示异常
    fusion_img = original_img_denorm.copy()
    for c in range(3):
        fusion_img[:, :, c] = np.where(
            sam_mask_np & (anomaly_map_np > 0.3),
            heatmap[:, :, c] * 0.7 + original_img_denorm[:, :, c] * 0.3,
        ...
        for sample_batched in tqdm(dataloader, desc="Evaluating"):
            ...
            if category in SPECIAL_CATEGORIES:
                for i, filename in enumerate(filenames):
                    ...
                    # 假设文件名格式为 "defect_typeXXX" (XXX为三位数字)
                    if len(filename) > 3:
                    ...
                    if os.path.exists(sam_mask_path):
                        ...
                        # 应用SAM掩码到分割输出
                        output_segmentation[i:i+1] = apply_sam_mask(
                            output_segmentation[i:i+1], sam_mask
                        )
                        
                        # 保存处理后的缺陷掩码
                        output_front_dir = os.path.join("./output", "output_front", category, defect_type)
                        os.makedirs(output_front_dir, exist_ok=True)
                        seg_mask_np = (output_segmentation[i].squeeze().cpu().numpy() * 255).astype(np.uint8)
                        Image.fromarray(seg_mask_np).save(os.path.join(output_front_dir, f"{file_idx}.png"))
                        ...
                        fusion_img = create_fusion_image(
                            img[i], 
                            output_segmentation[i],
                            sam_mask
                        )
