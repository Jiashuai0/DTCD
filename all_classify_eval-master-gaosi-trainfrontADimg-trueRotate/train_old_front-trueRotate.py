...
from constant import ALL_CATEGORY
...
def perlin_noise(image, dtd_image, aug_prob=1.0):
    ...
    img_thr = dtd_image.astype(np.float32) * perlin_thr / 255.0
    image = image / 255.0
...
def perlin_noise_with_sam_mask(image, dtd_image, sam_mask, aug_prob=1.0):
    ...
    sam_mask = np.where(sam_mask > 127, 0.0, 1.0)  # 反转掩码：前景为0，背景为1
...
class MVTecDatasetWithSAM(Dataset):
    ...
    def __getitem__(self, index):
        img_path = self.mvtec_paths[index]
        img = Image.open(img_path).convert("RGB")
        img = img.resize(self.resize_shape, Image.BILINEAR)
        
        if self.is_train:
            # 加载对应的SAM前景掩码
            if index < len(self.sam_paths) and os.path.exists(self.sam_paths[index]):
                sam_mask = Image.open(self.sam_paths[index]).convert("L")
                sam_mask = sam_mask.resize(self.resize_shape, Image.BILINEAR)
            else:
                # 创建全白掩码（背景）
                sam_mask = Image.new("L", self.resize_shape, color=255)
            
            # 保存SAM掩码的numpy数组形式用于可视化
            sam_mask_np = np.array(sam_mask)  # 形状为 (H, W)，值在0-255之间
            
            # 随机选择DTD纹理
            dtd_img_path = random.choice(self.dtd_paths)
            dtd_img = Image.open(dtd_img_path).convert("RGB")
            dtd_img = dtd_img.resize(self.resize_shape, Image.BILINEAR)
            
            # 在合成前应用旋转
            fill_color = (114, 114, 114)
            fill_color_mask = 0  # 掩码的填充颜色（黑色）
            rotation_degree = 0  # 记录旋转角度
            
            # rotate_90
            if self.rotate_90:
                rotation_degree = np.random.choice(np.array([0, 90, 180, 270]))
                img = img.rotate(
                    rotation_degree, fillcolor=fill_color, resample=Image.BILINEAR
                )
                # 对SAM掩码应用相同旋转
                sam_mask = sam_mask.rotate(
                    rotation_degree, fillcolor=fill_color_mask, resample=Image.NEAREST
                )
                # 更新旋转后的SAM掩码数组
            ...
            if self.random_rotate > 0:
                rotation_degree = np.random.uniform(-self.random_rotate, self.random_rotate)
                img = img.rotate(
                    rotation_degree, fillcolor=fill_color, resample=Image.BILINEAR
                )
                # 对SAM掩码应用相同旋转
            ...
            if self.classname in SPECIAL_CATEGORIES:
                # 使用带SAM掩码的Perlin噪声生成方法
                aug_img_np, aug_mask_np = perlin_noise_with_sam_mask(
                    np.array(img), 
                    np.array(dtd_img),
                    np.array(sam_mask),
                    aug_prob=1.0
                ...
                # 使用原始的Perlin噪声生成方法
                aug_img_np, aug_mask_np = perlin_noise(
                    np.array(img), 
                    np.array(dtd_img),
                    aug_prob=1.0
                )
            ...
            # 转换为PIL图像
            aug_img = (aug_img_np * 255).astype(np.uint8)
            aug_img = Image.fromarray(aug_img)
            
            # 处理掩码 (1, H, W) -> (H, W)
            aug_mask = (aug_mask_np[0] * 255).astype(np.uint8)
            aug_mask = Image.fromarray(aug_mask)
            
            # 应用预处理
            img_tensor = self.final_preprocessing(img)
            aug_img_tensor = self.final_preprocessing(aug_img)
            aug_mask_tensor = torch.from_numpy(aug_mask_np[0]).float()
            
            return {
                "img_origin": img_tensor,
                "img_aug": aug_img_tensor,
                "mask": aug_mask_tensor,
                # 新增：返回用于可视化的原始数据
                "img_np": np.array(img),           # 原始图像数组 (H, W, 3)
                "aug_img_np": aug_img_np * 255,   # 合成异常图像数组 (H, W, 3) 0-255
                "sam_mask_np": sam_mask_np,        # SAM掩码数组 (H, W) 0-255
                "aug_mask_np": aug_mask_np[0] * 255,  # 合成异常掩码数组 (H, W) 0-255
            }
        else:
            img = self.final_preprocessing(img)
            ...
            return {"img": img, "mask": mask, "file": file}
...
def train(args, category, rotate_90=False, random_rotate=0):
    ...
    while flag:
        for _, sample_batched in enumerate(dataloader):
            ...
            # 每100个step保存一次batch数据
            if global_step % 1000 == 0:
                save_dir = f"./output/augimg/step_{global_step}/{category}"
                ...
                for i in range(batch_size):
                    # 获取各种图像数据
                    img_np = sample_batched["img_np"][i].numpy().astype(np.uint8)  # 原始图像
                    aug_img_np = sample_batched["aug_img_np"][i].numpy().astype(np.uint8)  # 合成异常图像
                    sam_mask_np = sample_batched["sam_mask_np"][i].numpy().astype(np.uint8)  # SAM掩码
                    aug_mask_np = sample_batched["aug_mask_np"][i].numpy().astype(np.uint8)  # 合成异常掩码
                    
                    # 保存原始图像
                    img_path = os.path.join(save_dir, f"{i}_origin.png")
                    cv2.imwrite(img_path, cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR))
                    
                    # 保存合成异常图像
                    aug_img_path = os.path.join(save_dir, f"{i}_aug.png")
                    cv2.imwrite(aug_img_path, cv2.cvtColor(aug_img_np, cv2.COLOR_RGB2BGR))
                    
                    # 保存SAM掩码
                    sam_mask_path = os.path.join(save_dir, f"{i}_sam_mask.png")
                    cv2.imwrite(sam_mask_path, sam_mask_np)
                    
                    # 保存合成异常掩码
                    aug_mask_path = os.path.join(save_dir, f"{i}_aug_mask.png")
                    cv2.imwrite(aug_mask_path, aug_mask_np)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resize", type=int, default=256)  # 添加resize参数
    ...
    if args.custom_training_category:
        ...
        no_rotation_category = [
            "capsule", "metal_nut", "pill", "toothbrush", "transistor",
        ]
        slight_rotation_category = [
            "wood", "zipper", "cable",
        ]
        rotation_category = [
            "bottle", "grid", "hazelnut", "leather", "tile", "carpet", "screw",
        ]
