    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            if self.rotate_90:
                ...
                rotation_params = ("rotate_90", degree)
                image = image.rotate(
            ...
            if self.random_rotate > 0:
                ...
                rotation_params = ("random_rotate", degree)
                image = image.rotate(
                    degree, fillcolor=fill_color, resample=Image.BILINEAR
                )
            ...
            if self.classname in SPECIAL_CATEGORIES:
                ...
                # 应用相同的旋转到SAM掩码
                if rotation_params:
                    rotation_type, degree = rotation_params
                    if rotation_type == "rotate_90":
                        # 90度旋转需要特殊处理以确保旋转角度精确
                        sam_mask = sam_mask.rotate(degree, fillcolor=0, resample=Image.BILINEAR)
                    else:
                        sam_mask = sam_mask.rotate(degree, fillcolor=0, resample=Image.BILINEAR)
            ...
            return {"img_aug": aug_image, "img_origin": image, "mask": aug_mask, "aug_image_np": np.array(aug_image)}
        else:
            image = self.final_preprocessing(image)
...
#定义一个保存增强后图像和掩码的函数
def save_augmented_images(sample_batched, step, save_dir="./output/augimg"):
    """保存增强后的图像和掩码"""
    os.makedirs(save_dir, exist_ok=True)
    
    # 获取当前batch的数据
    img_origin = sample_batched["img_origin"]  # 原始图像（已旋转）
    img_aug = sample_batched["img_aug"]        # 合成异常图像
    mask = sample_batched["mask"]              # 合成异常掩码
    ...
    # 保存每个batch中的图像
    for i in range(img_origin.size(0)):
        # 原始图像（旋转后的正常图像）
        orig_img = denormalize(img_origin[i]).clamp(0, 1).permute(1, 2, 0).numpy() * 255
        orig_img = orig_img.astype(np.uint8)
        
        # 合成异常图像
        aug_img = denormalize(img_aug[i]).clamp(0, 1).permute(1, 2, 0).numpy() * 255
        aug_img = aug_img.astype(np.uint8)
        
        # 合成异常掩码
        aug_mask = mask[i].permute(1, 2, 0).numpy() * 255
        aug_mask = aug_mask.astype(np.uint8)
        
        # 保存图像
        cv2.imwrite(os.path.join(save_dir, f"step{step}_orig_{i}.png"), orig_img[..., ::-1])  # RGB转BGR
        cv2.imwrite(os.path.join(save_dir, f"step{step}_aug_{i}.png"), aug_img[..., ::-1])   # RGB转BGR
        cv2.imwrite(os.path.join(save_dir, f"step{step}_mask_{i}.png"), aug_mask)
        
        # 如果SAM掩码可用，也保存它
        if "sam_mask" in sample_batched:
            sam_mask = sample_batched["sam_mask"][i].numpy() * 255
            sam_mask = sam_mask.astype(np.uint8)
            cv2.imwrite(os.path.join(save_dir, f"step{step}_sam_{i}.png"), sam_mask)
    ...
    while flag:
        for _, sample_batched in enumerate(dataloader):
            ...
            if global_step % 100 == 0:
                save_augmented_images(sample_batched, global_step)
            """
